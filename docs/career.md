...
AI 기능과 별개로, 포털 이용자가 BI 서비스를 끊김 없이 사용할 수 있도록 포털-BI 연동 구간의 기능도 함께 확장했습니다.

- **SSO 연동**: 포털 로그인 세션을 BI 서비스가 그대로 인식할 수 있도록 SSO 인증 흐름을 연동
- **드릴다운(Drill Down) 기능**: 요약 지표에서 상세 데이터로 바로 이동할 수 있는 드릴다운 조회 기능 개발
- **계산된 컬럼 · 총계 별도 계산**: 사용자 정의 계산식 컬럼과, 전체 합계를 개별 항목과 다른 기준으로 계산하는 "총계 별도 계산" 옵션 개발
- **변동측정값 표시 개선**: 전기 대비 증감량/증감률을 함께 보여주는 변동측정값의 소수점 자리수·표시 여부를 사용자가 설정할 수 있도록 개선
...
**기간** 2025.06 ~ 2026.02 · **소속** ㈜위세아이텍 WI Development Team · **역할** Frontend 개발
**기술 스택** React, DevExtreme(PivotGrid/DataGrid), SpreadJS, Spring

### 프로젝트 개요

스포츠토토 BI 서비스에 SSO(Single Sign-On) 인증을 연동하고, 피벗그리드·데이터그리드·SpreadJS 기반 보고서 기능의 안정성을 높인 유지보수 및 기능 개발 프로젝트입니다.
...
성능 개선과 병행해, 보고서 조회 편의성을 높이는 기능도 지속적으로 추가했습니다.

- **보고서/폴더 Drag & Drop 관리**: 개인·공용 보고서와 폴더를 트리 구조에서 자유롭게 이동할 수 있는 Drag & Drop 기능 개발
- **즐겨찾기 기능**: 자주 조회하는 보고서를 즐겨찾기로 등록·조회할 수 있는 기능 추가
- **틀 고정(Freeze Panes)**: 피벗그리드에서 특정 행/열을 고정한 채 나머지 영역만 스크롤할 수 있는 기능 개발, 해제 시 내부 스크롤 활성화로 로드 속도 개선
- **엑셀 업로드/다운로드 권한 분리**: 엑셀 업로드·리소스맵 기능에 별도 권한 체계를 적용해 기능 접근을 세분화
- **쿼리 검색 기능**: 사용자가 작성한 쿼리를 기준으로 보고서를 검색할 수 있는 기능 개발
