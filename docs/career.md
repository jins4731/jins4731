...
5. [국방부 통계분석 기능 개발](#defense-stats) `2024 ~ 2025`
6. [BI 플랫폼 레거시 현대화](#legacy-modernization) `2022.07 ~ 2023.12`
7. [산림청 산림공간정보 GIS 시각화](#forest-service-gis) `2022 ~ 2023`
...
<a id="defense-stats"></a>

## 5. 국방부 통계분석 기능 개발

**기간** 2024 ~ 2025 · **소속** ㈜위세아이텍 WI Development Team · **역할** Frontend 개발
**기술 스택** React, DevExtreme, Java(백엔드 통계 엔진), R

### 프로젝트 개요

국방부 대상 구축형 BI 서비스에 통계 분석 기능을 추가하는 프로젝트입니다. 사용자가 데이터를 업로드하면 선형회귀·F검정 등 통계 기법을 선택해 분석하고, 그 결과를 화면에서 바로 확인할 수 있도록 했습니다. 통계 연산은 Java 통계 라이브러리와 R 엔진이 백엔드에서 처리하고, 프론트엔드는 분석 조건 설정부터 결과 시각화까지 담당했습니다.

### 담당 역할

- 데이터 업로드 및 통계 기법(선형회귀, F검정 등) 설정 UI 개발
- R·Java 통계 엔진의 연산 결과를 차트·테이블로 가공하는 결과 화면 구현
- 백엔드 통계 엔진과의 API 연동 구조 설계

### 해결한 문제

**통계 프로그램 없이는 분석이 어려운 환경**
통계 분석을 하려면 R이나 SPSS 같은 별도 프로그램을 다룰 줄 알아야 했고, 이는 통계 지식이 없는 실무자에게 진입장벽이었습니다.

- BI 화면에서 데이터 업로드부터 통계 기법 선택, 결과 확인까지 이어지는 흐름을 구현해 별도 프로그램 없이 분석을 완결할 수 있도록 지원

### 구현 내용

- 선형회귀 계수, F검정 통계량 등 통계 분석 결과를 표·차트로 가공해 표시
- 통계 기법별로 다른 입력 파라미터와 결과 구조에 대응하는 유연한 UI 구조 설계

### 성과

- 별도 통계 프로그램 없이 BI 화면 안에서 통계 분석을 완결할 수 있는 환경 제공
- 통계 지식이 부족한 사용자도 정형화된 UI로 분석 기법을 선택·실행할 수 있도록 진입장벽 완화
