...
6. [산림청 산림공간정보 GIS 시각화](#forest-service-gis) `2022 ~ 2023`
7. [문화정보원 관광 통계 지도 시각화](#tourism-map) `2023`
8. [BI 데이터 설계 플랫폼 개발](#data-design-platform) `2020.07 ~ 2022.06`
...
<a id="forest-service-gis"></a>

## 6. 산림청 산림공간정보 GIS 시각화

**기간** 2022 ~ 2023 (약 2개월) · **소속** ㈜위세아이텍 WI Development Team · **역할** Frontend 개발
**기술 스택** React, DevExtreme(VectorMap), GeoJSON, SDF

### 프로젝트 개요

산림청 대상 구축형 BI 서비스에 GeoJSON·SDF 기반 행정구역 지도 시각화 기능을 개발한 프로젝트입니다. 행정동·법정동 경계가 다르고 시군구 통폐합 등으로 시기별 행정구역이 달라지는 한국 행정구역 특성을 반영해, 시도 → 시군구 → 읍면동으로 이어지는 드릴다운 지도를 구현했습니다.

### 담당 역할

- DevExtreme VectorMap을 GeoJSON/SDF 데이터 기반으로 커스터마이징
- 시도·시군구·읍면동 행정구역 코드를 계층적으로 매핑해 드릴다운 지도 인터랙션 구현
- 행정구역 개편 시점에 대응하는 GeoJSON 경계 데이터 갱신 구조 마련

### 해결한 문제

**행정구역 체계의 복잡성과 시기별 변경**
한국 행정구역은 행정동과 법정동 경계가 다르고, 시군구 통폐합·명칭 변경이 주기적으로 발생해 고정된 경계 데이터로는 정확한 지도 시각화가 불가능했습니다.

- 시도·시군구·읍면동 행정구역 코드 체계를 직접 매핑하는 로직을 구현해, 상위 행정구역을 클릭하면 하위 구역으로 전환되는 드릴다운 지도 구성
- 행정구역 개편 시점에 맞춰 GeoJSON 경계 데이터를 갱신할 수 있는 구조로 설계

### 구현 내용

- DevExtreme VectorMap에 GeoJSON/SDF 포맷의 행정구역 경계 데이터 바인딩
- 시도 → 시군구 → 읍면동 3단계 드릴다운 인터랙션 구현

### 성과

- 표준화되지 않고 주기적으로 변경되는 행정구역 데이터를 안정적으로 반영하는 지도 시각화 구조 확보
- 읍면동 단위까지 세분화된 공간 통계 시각화 제공

---

<a id="tourism-map"></a>

## 7. 문화정보원 관광 통계 지도 시각화

**기간** 2023 (약 2~3개월) · **소속** ㈜위세아이텍 WI Development Team · **역할** Frontend 개발
**기술 스택** React, Kakao Map API, Google Maps API

### 프로젝트 개요

문화정보원 대상으로 지역별 관광 통계를 지도 위에 시각화한 독립 프로젝트입니다. 마커·폴리곤으로 지역별 통계를 표현하는 것에서 나아가, 관광객의 방문 순서·동선을 노드·경로로 시각화하고 방문 이력을 기반으로 다음 경로를 추천하는 기능까지 개발했습니다.

### 담당 역할

- Kakao Map/Google Maps API 기반 지역별 관광 통계 마커·폴리곤 시각화 개발
- CustomOverlay·Polyline 기반 관광객 동선 노드·경로 시각화 구현
- 관광지 방문 이력 기반 이동 경로 추천 기능 개발

### 해결한 문제

**Kakao Map SDK 미지원 경로 시각화, 경로 중첩 시 성능·가독성 저하**
관광객의 동선을 지도 위에 노드·경로로 표현하려 했지만, Kakao Map SDK는 이런 형태의 커스텀 오버레이를 기본으로 지원하지 않았습니다. 여러 경로가 동시에 표시될 때는 렌더링 성능이 떨어지고, 경로가 서로 겹쳐 보여 가독성도 함께 낮아졌습니다.

- CustomOverlay와 Polyline을 조합해 노드·경로 오버레이를 직접 구현하고, 데이터 변경 시 전체를 다시 그리지 않고 변경된 오버레이만 선택적으로 갱신
- 화면에 필요한 경로만 렌더링하고, 더 이상 쓰지 않는 Polyline은 `setMap(null)`로 명시적으로 제거해 메모리 누수 방지
- 선택된 경로는 색상·두께를 다르게 표시해 다수 경로가 겹치는 상황에서도 가독성 확보

### 구현 내용

- 지역별 관광 통계를 마커·폴리곤으로 시각화하는 지도 컴포넌트 개발
- 방문 순서 데이터를 노드-엣지 구조로 변환해 지도 위에 렌더링
- 관광지 방문 이력을 분석해 다음 방문지를 추천하는 로직 구현

### 성과

- 통계 수치를 넘어 관광객의 실제 동선을 지도 위에서 직관적으로 파악할 수 있는 시각화 제공
- 다수 경로가 겹치는 상황에서도 성능 저하와 가독성 저하 없이 안정적으로 시각화하는 구조 확보
- 방문 이력 기반 경로 추천 기능으로 관광 데이터의 활용도 향상
